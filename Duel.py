...
class DQNAgent:  # Define DQNAgent class
    def __init__(self, state_size, action_size,  # Initialize method
                 memory_size=2000,  # Set memory size for experience replay
                 ...
                 ddqn_flag=False,  # Set whether to use double deep Q network
        ...
        self.memory_size = memory_size  # Set memory size for experience replay
        ...
        self.model = self._build_model()  # Build main model
        self.target_model = self._build_model()  # Build target model
        ...
        self.target_model.set_weights(self.model.get_weights())  # Initialize target model weights with main model weights

    def _build_model(self):  # Define method to build model
        # Advantage network
        ...
        if self.dueling_option == 'avg':  # If using average Dueling architecture
            network_output = Lambda(lambda x: x[0] - tf.reduce_mean(x[0]) + x[1], output_shape=(self.action_size,))([A3, V3])  # Compute Q values
        elif self.dueling_option == 'max':  # If using maximum Dueling architecture
            network_output = Lambda(lambda x: x[0] - tf.reduce_max(x[0]) + x[1], output_shape=(self.action_size,))([A3, V3])  # Compute Q values
        elif self.dueling_option == 'naive':  # If using naive Dueling architecture
            network_output = Lambda(lambda x: x[0] + x[1], output_shape=(self.action_size,))([A3, V3])  # Compute Q values
        else:  # If Dueling architecture type is invalid
            raise Exception('Invalid Dueling Option')  # Raise an exception

        model = Model(network_input, network_output)  # Create model
        model.compile(loss='mse', optimizer=Adam(lr=self.learning_rate))  # Compile model
        model.summary()  # Print model summary
        return model  # Return model

    def update_target_network(self):  # Define method to update target network
        if self.ddqn and self.polyak_avg:  # If using double deep Q network and Polyak averaging
            ...
            for i in range(len(target_weights)):  # Iterate through weights
            ...
            self.target_model.set_weights(self.model.get_weights())  # Set target model weights to main model weights
    ...
    def select_action(self, state):  # Define method to select action
        if random.random() < self.epsilon:  # Select action based on exploration rate
    ...
    def get_maxQvalue_nextstate(self, next_state):  # Define method to get maximum Q-value of next state
        max_q_value = np.amax(self.target_model.predict(next_state)[0])  # Get maximum Q-value selected by target model in next state
    ...
    def train_model(self):  # Define method to train model
        ...
        for i in range(self.batch_size):  # Iterate through batch size
            ...
            qValues[i] = self.model.predict(current_state[i].reshape(1, self.state_size))[0]  # Get Q-values of current state
            max_qvalue_ns = self.get_maxQvalue_nextstate(next_state[i].reshape(1, self.state_size))  # Get maximum Q-value of next state
...
Scores = []  # Create list for scores
AvgScores = []  # Create list for average scores
Avg100Scores = []  # Create list for last 100 average scores

for e in range(max_episodes):  # Loop through episodes
    ...
    while not done:  # Loop until done
        ...
        if done:
            ...
            print('Episode: {}, time steps: {}, AvgScore: {:0.2f}, epsilon: {:0.2f}, replay size: {}'.format(e, t, np.mean(Scores), deepQ.epsilon, len(deepQ.memory)))  # Print message

    if np.mean(last100Scores) > (env.spec.max_episode_steps - 5):  # If last 100 average scores is greater than max steps minus 5
...
# Plot time steps per episode-episode graph
plt.subplot(1, 3, 2)
plt.plot(Scores)
plt.xlabel('Episode')
...
# Plot average score per episode-episode graph
plt.subplot(1, 3, 3)
plt.plot(AvgScores)
plt.xlabel('Episode')
